...
// TestInputActionTimeout tests that timeout mechanism works correctly
func TestInputActionTimeout(t *testing.T) {
	// Setup minimal environment
	n := 2
	listeners, addresses := network.CreateListeners(n)
	defer func() {
	...
	errChan := make(chan error, n)
	actionChan := make(chan bool, n)

	for i := 0; i < n; i++ {
		go func(idx int) {
			session := poker.Session{
				Players:     players,
				CurrentTurn: uint(idx),
				Round:       poker.PreFlop,
				HighestBet:  0,
				Pots:        []poker.Pot{{Amount: 0, Eligible: []int{0, 1}}},
			}
			...
			// This should timeout and automatically check/fold
			// We can't actually test the UI part, but we can verify the timeout logic
			// by checking that the function returns without hanging
			done := make(chan error, 1)
			go func() {
				done <- inputAction(psm, *node, idx)
			}()
			...
			case <-time.After(timeout + 3*time.Second):
				errChan <- fmt.Errorf("inputAction hung, timeout mechanism failed")
			}
		}(i)
	}
	...
	for i := 0; i < n; i++ {
		if err := <-errChan; err != nil {
			t.Logf("Expected behavior or error: %v", err)
		}
	}
}

// TestInputActionNonPlayerTurn tests waiting for other player's action
func TestInputActionNonPlayerTurn(t *testing.T) {
	n := 2
	listeners, addresses := network.CreateListeners(n)
	defer func() {
		for _, l := range listeners {
	...
	players := []poker.Player{
		{Id: 0, Name: "Alice", Pot: 100, Bet: 0, HasFolded: false},
		{Id: 1, Name: "Bob", Pot: 100, Bet: 0, HasFolded: false},
	}
	...
	for i := 0; i < n; i++ {
		go func(idx int) {
			...
			go func() {
				// For non-current player, this should wait
				// For current player, this will timeout and auto-action
				done <- inputAction(psm, *node, idx)
			}()
			...
			case <-time.After(8 * time.Second):
				// Timeout expected for waiting player
				if idx != 0 {
					errChan <- nil // Expected for non-current player
				} else {
					errChan <- fmt.Errorf("current player should have completed")
				}
			}
		}(i)
	}
	...
	for i := 0; i < n; i++ {
		err := <-errChan
		if err != nil {
			t.Logf("Result from player %d: %v", i, err)
		}
	}
}
