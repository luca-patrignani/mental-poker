...
\begin{itemize}
\item \textbf{Elliptic curve support}: Ed25519 curve operations\cite{ed25519} required by the Wei \& Wang protocol.
...
The solution was designed with five main components:
...
\item \textbf{The discovery service}: Service for announcing and discovering peers in the local network
...
\begin{itemize}
\item \textbf{Stateless transitions}: Each action transforms one complete game state into another, making the system naturally stateless between actions.
\item \textbf{Uniform interface}: All state transitions follow the same consensus-vote-append pattern.
\item \textbf{Self-descriptive messages}: Each block in the replicated log is self-contained.
\item \textbf{Hypermedia as the engine of application state (HATEOAS)}: The blockchain structure naturally implements this principle - each block links to the previous block via hash.
\end{itemize}

\subsubsection{Possible Transition to HTTP-based Implementation}
...
The validation strategy employs automated testing for unit tests and integration testing.
