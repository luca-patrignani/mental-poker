...
\author{Galeri Marco \\ \emailaddr{marco.galeri@studio.unibo.it}
\and Patrignani Luca \\ \emailaddr{luca.patrignani3@studio.unibo.it}}
\date{\today}
...
Computer science students learning about cryptographic protocols use the mental poker implementation as a practical example. They can examine the source code to understand how the Wei \& Wang protocol works\cite{Wei-Wang-Protocol}, run games while monitoring network traffic to see the cryptographic operations, and modify the code to experiment with different protocol parameters or attack scenarios (demonstrating security guarantees when attempting to cheat).
...
\item A golang library (\texttt{ledger}) implementing a blockchain-based distributed ledger for recording player actions with consensus voting mechanisms\cite{eventtourcing}.
...
The project uses \textbf{Kyber}\cite{kyberlib}, a Go cryptography library providing elliptic curve operations. Specifically, the Ed25519 curve suite\cite{ed25519} is used for:
...
\item \textbf{Homomorphic properties}: Enabling multiple players to collectively encrypt the deck without revealing individual secret keys\cite{homomorphic-encryption}
\end{itemize}

The Wei \& Wang protocol leverages the discrete logarithm problem on elliptic curves to provide computational security guarantees\cite{discrete-log-problem}.
...
The player is able to play a game of Texas Hold'em poker following standard rules\cite{texasholdem}:
...
\item \textbf{Cryptographic Security}: The protocols provide computational security based on the discrete logarithm problem\cite{discrete-log-problem}. Breaking the security would require solving computationally infeasible problems.
...
\item \textbf{Byzantine Fault Tolerance}: The game continues correctly even when some players behave maliciously (within the $\frac{2}{3} k$ coalition limit)\cite{bfttheory}.
...
The \textbf{MPI-inspired broadcast primitives}\cite{mpi41} were chosen because:

\begin{itemize}
\item \textbf{Protocol compatibility}: The Wei \& Wang paper\cite{Wei-Wang-Protocol} describes operations that naturally map to broadcast and all-to-all patterns.
...
\textbf{Go (Golang)}\cite{golang} was selected as the implementation language because:
...
The \textbf{Kyber cryptography library}\cite{kyberlib} (Dedis Advanced Crypto Library for Go) provides:

\begin{itemize}
\item \textbf{Homomorphic cryptography}: Kyber supports various suites of homomorphic cryptographic encryptions such as curve operations\cite{ed25519} required by the Wei \& Wang protocol.
...
\begin{figure}
\centering
\includegraphics[width=1.1\linewidth]{figures//package-uml.png}
...
\begin{itemize}
\item \textbf{The Deck}: Cryptographic layer implementing the Wei \& Wang mental poker protocol\cite{Wei-Wang-Protocol}

\item \textbf{The Game Logic}: Poker game mechanics, state management, and rules enforcement

\item \textbf{The DLT (Distributed Ledger Technology)}: Event-sourcing blockchain\cite{eventtourcing} to record actions

\item \textbf{The Consensus Node}: Voting mechanisms for action validation\cite{bfttheory}
...
\begin{itemize}
  ...
  \item \textbf{Discovery Service}: Service for announcing and discovering peers in the local network
\end{itemize}
...
The Deck component implements the cryptographic layer based on the Wei \& Wang mental poker protocol\cite{Wei-Wang-Protocol}. It ensures that no player can control or predict the card order, and that cards dealt face-down remain private until revealed.

\paragraph{Deck Entity}

The \texttt{Deck} struct is the core cryptographic abstraction representing a secure card deck. Each player maintains their own Deck instance, participating in collective protocols for shuffling, dealing, and revealing cards. The deck uses elliptic curve cryptography (Ed25519)\cite{ed25519} through the Kyber library\cite{kyberlib}, where each card is represented as a point on the curve. Cards are encrypted with each player's secret key during the shuffle protocol, ensuring that the final deck order incorporates randomness from all participants.

Key operations include:

\begin{itemize}
\item \texttt{PrepareDeck()}: Initializes $n$ cards as cryptographic points using distributed random element generation
...
The Poker component manages game state, enforces Texas Hold'em rules\cite{texasholdem}, and evaluates hand strengths. It operates independently of the cryptographic deck, focusing purely on game mechanics.

\paragraph{Session Entity}

The \texttt{Session} struct maintains the complete game state at any point in time. It tracks community cards on the board, player states, pot distributions, betting information, and the current game phase (PreFlop, Flop, Turn, River, Showdown). The session serves as the single source of truth for game state.
...
The \texttt{Card} struct represents standard playing cards with rank and suit. The \texttt{Evaluator} module implements poker hand ranking algorithms\cite{texasholdem}, comparing hands to determine winners according to standard Texas Hold'em rules (Royal Flush through High Card).

\paragraph{Action Entity}

The \texttt{Action} struct represents a proposed poker move with a unique ID, player ID, poker action payload, timestamp, and Ed25519 signature\cite{ed25519}.

\begin{figure}
\centering
\includegraphics[width=0.9\linewidth]{figures//domain-uml.png}
...
The Ledger component implements an event-sourced\cite{eventtourcing}, blockchain-based replicated log ensuring immutability and non-repudiation of all game actions. 

\paragraph{Blockchain Entity}

The \texttt{Blockchain} struct maintains an append-only chain of blocks, each cryptographically linked to its predecessor via SHA-256 hashing\cite{sha256}. The genesis block records the initial game state with all players and starting chip counts. Each subsequent block contains a player action, the resulting game state, consensus votes, and metadata.
...
\begin{figure}
\centering
\includegraphics[width=0.7\linewidth]{figures//ledger-uml.png}
...
The Consensus component implements a Byzantine Fault Tolerant (BFT) voting protocol\cite{bfttheory} ensuring that all players agree on action validity before state changes occur.

\begin{figure}
\centering
\includegraphics[width=0.8\linewidth]{figures//consensus-uml.png}
\caption{UML class diagram showing the consensus component details}
\label{fig:class-diagram}
\end{figure}

\paragraph{Node Entity}

The \texttt{ConsensusNode} manages the distributed consensus protocol for each player. It maintains cryptographic key pairs (Ed25519)\cite{ed25519} for signing and verifying messages, tracks peer public keys, calculates quorum thresholds using the BFT formula\cite{bfttheory} $\lceil\frac{2n+2}{3}\rceil$, and coordinates with the poker state machine and blockchain ledger.
...
The \texttt{Vote} struct contains the voter's decision (ACCEPT/REJECT), validation reason, and cryptographic signature\cite{ed25519}. All signatures are verified using the voter's public key to prevent forgery.

\subsubsection{Network Component}

The Network component provides peer-to-peer communication primitives\cite{p2p-networks} abstracting the underlying TCP connections.

\paragraph{NetworkLayer Interface}

The \texttt{NetworkLayer} interface defines two fundamental communication patterns inspired by MPI\cite{mpi41}:
...
\subsubsection{The discovery service}
...
As previously mentioned, each player communicates only via two primitives\cite{mpi41}:
...
\vspace*{-2cm}
\centering
\includegraphics[width=1.3\textwidth,height=1.1\textheight,keepaspectratio]{figures//consensus-protocol-sequence.png}
...
The player whose turn it is creates a poker action (e.g., CHECK, FOLD, RAISE) and submits it. The node generates a unique action ID by combining the player ID, the action payload hash, and 128 bits of cryptographic entropy. The action is timestamped and signed with the player's private key through the elliptic curve cryptography\cite{ed25519}. The signed action is then broadcast to all peers using the \texttt{Broadcast} primitive.

All other players are executing \texttt{WaitForProposal()}, blocking until they receive the broadcast from the current player or the default action in case of timeout.

\paragraph{Phase 2: Proposal Validation}

Upon receiving a proposal, each consensus node locally validates it:

\begin{enumerate}
\item \textbf{Signature verification}: The node verifies the signature using the proposer's public key to ensure authenticity and prevent tampering.
\item \textbf{Game rule validation}: The proposal is passed to the poker manager, which validates turn order, action legality (e.g., sufficient chips for a bet) and compliance with Texas Hold'em rules\cite{texasholdem}.
\end{enumerate}

If any validation check fails, the node prepares to vote \texttt{REJECT} with a descriptive reason (e.g., ``out-of-turn'', ``insufficient-chips'', ``bad-signature''). Otherwise, the node votes with \texttt{ACCEPT}.
...
\item A signature on the serialized vote data
\end{itemize}
...
\item \textbf{Signature verification}: For each vote, verify the Ed25519 signature\cite{ed25519} using the voter's public key from the peer map.
...
The \texttt{checkAndCommit()} function evaluates whether consensus has been reached. The quorum threshold is computed using the Byzantine Fault Tolerance formula\cite{bfttheory}:
...
\begin{itemize}
\item \textbf{Accepts $\geq$ quorum}: The action is legitimate. The consensus node applies the action to the poker state machine, appends a block to the blockchain\cite{eventtourcing}, and clears cached proposal and votes.

\item \textbf{Rejects $\geq$ quorum}: The proposal is invalid and the proposer is Byzantine. The consensus node bans the player, records the ban action in the blockchain\cite{eventtourcing}, and recalculates quorum.

\item \textbf{Neither quorum reached}: Insufficient votes have been collected. The function returns an error, and the node waits for more votes to arrive.
\end{itemize}

Even if some nodes are Byzantine (sending different votes or proposals), the quorum mechanism ensures that the supermajority of honest nodes drives the consensus outcome\cite{bfttheory}. The blockchain records the complete set of votes, enabling post-game auditing to identify Byzantine actors.
...
Message serialization uses JSON encoding for human readability and debugging convenience and then in binary to reduce bandwidth. The communication model follows the \textbf{Single Program, Multiple Data (SPMD)} paradigm\cite{spmd} from parallel computing: all nodes run identical code but make different execution decisions based on their node ID and the current game state. For example, only the node whose turn it is executes \texttt{ProposeAction()}, while others execute \texttt{WaitForProposal()}.
...
\begin{figure}
\centering
\includegraphics[width=1\linewidth]{figures//poker-state-machine.png}
\caption{Finite state machine of a Texas Hold-em poker game}
\label{fig:class-diagram}
\end{figure}
...
The system employs a \textbf{permissioned blockchain} as a distributed ledger to record all game actions immutably. This ensures transparency, auditability, and non-repudiation of player actions. Since the set of players is known and controlled, a permissioned model is appropriate.
A \textbf{replicated log} similar to a \textit{blockchain} data structure keeps track of the state of the game in an \textbf{event sourcing} fashion\cite{eventtourcing}, recording all game actions with consensus verification. Each block contains\cite{sha256}:
...
This provides \textbf{non-repudiation}, so players cannot deny actions they performed, as all actions are cryptographically signed and verified by the group\cite{bfttheory}. The replicated log also enables post-game auditing and dispute resolution.
...
\begin{itemize}
\item \textbf{Malicious Stalling}: Timeouts on broadcast/all-to-all operations detect stalling players
\item \textbf{Invalid cryptographic operations}: Card verification failures detect attempts to reveal fake cards
\item \textbf{Consensus violations}: Actions without sufficient votes are rejected\cite{bfttheory}
\item \textbf{Blockchain integrity violations}: \texttt{Verify()} method detects tampered history\cite{sha256}
\item \textbf{Protocol violations}: Invalid game actions are rejected
\end{itemize}
...
\begin{itemize}
\item \textbf{Detected cheating}: Malicious player is banned via consensus vote\cite{bfttheory}
...
\begin{enumerate}
\item Player signals intent to leave (via CLI)
\item The current hand completes if in progress
\item The Player's \texttt{LeaveGame()} method is called
\item The Player's encryption is removed from the deck
\item Network connections are closed
\item Application exits cleanly
\end{enumerate}

\section{Salient implementation details}

This section highlights key implementation details extracted from the actual codebase, demonstrating how theoretical protocols are realized in Go\cite{golang}.

\subsection{Cryptographic Deck Implementation}

The deck uses Kyber's\cite{kyberlib} Ed25519 elliptic curve suite\cite{ed25519}. Each card is represented as a cryptographic point, with randomness contributed by all players to prevent manipulation.

\begin{lstlisting}[language=Go, caption=Distributed random element generation from deck.go]
...
The \texttt{generateRandomElement()} method implements Protocol 2 from the Wei \& Wang paper\cite{Wei-Wang-Protocol}. Each player contributes randomness through \texttt{AllToAll}\cite{mpi41} exchanges of cryptographic points, ensuring that no single player can predict or control the final card values.

\subsection{Shuffle Protocol}

The shuffle implementation follows Protocol 3 from the Wei \& Wang paper\cite{Wei-Wang-Protocol}, where each player sequentially encrypts and permutes the deck.

\subsection{Card Drawing Protocol}

Drawing a card requires collective decryption, allowing only the designated player to learn the card value using Ed25519 elliptic curve cryptography\cite{ed25519}.

\subsection{Documentation}

Comprehensive GoDoc documentation\cite{golang} is provided for all public APIs. Examples:
...
The documentation can be found on the official go module \href{https://pkg.go.dev/github.com/luca-patrignani/mental-poker/v3}{site}.

\section{Validation}

\subsection{Testing Strategy}

The testing strategy employs automated testing for unit tests and integration testing.

\subsection{Test Coverage}

Test coverage statistics demonstrate comprehensive validation across all components.

\subsection{Manual Testing}

Manual testing scenarios include 3-player games, exiting games, and malicious stalling scenarios.

\subsection{Compliance with Requirements}

Validation demonstrates compliance with all requirements\cite{Wei-Wang-Protocol}\cite{bfttheory}:

\begin{itemize}
\item \textbf{Shuffle fairness}: Tested via statistical analysis of shuffle outcomes
\item \textbf{Card privacy}: Tested via cryptographic verification
\item \textbf{Card verifiability}: Tested via verification of revealed cards
\item \textbf{Non-repudiation}: Tested via blockchain immutability checks\cite{sha256}
\item \textbf{Consensus}: Tested via vote counting and quorum enforcement\cite{bfttheory}
\end{itemize}
...
Prebuilt binaries for end users are available for download in the release section of the GitHub repository.
...
\begin{itemize}
\item \textbf{Go}\cite{golang}: Version 1.23.5 or later
\item \textbf{Git}: For cloning the repository
\item \textbf{Network}: All players must be on the same local network
\end{itemize}
...
\item Install dependencies: \texttt{go mod download}
\item Build the executable: \texttt{go build -o mental-poker ./cmd}
\item (Optional) Install globally: \texttt{go install ./cmd}
\end{enumerate}

\section{Conclusions}

This project successfully implemented a decentralized poker game meeting all specified security and functionality requirements using the Wei \& Wang mental poker protocol\cite{Wei-Wang-Protocol}, Byzantine Fault Tolerant consensus\cite{bfttheory}, blockchain-based event sourcing\cite{eventtourcing}, and Go programming language\cite{golang}.

\subsection{Achievements}

The system enables trustless poker gameplay among multiple players without requiring a central authority, with all cryptographic primitives\cite{ed25519} properly implemented and validated.
...
\begin{itemize}
\item \textbf{Graphical user interface}: A web-based or desktop GUI would improve usability
\item \textbf{Chip transactions}: A cryptocurrency or token system for real-money betting
\item \textbf{Wide-area network support}: Extending beyond LAN to internet-scale with NAT traversal
\item \textbf{Zero-knowledge proofs}: More efficient card verification\cite{homomorphic-encryption}
\item \textbf{Mobile clients}: iOS and Android applications
\item \textbf{Tournament management}: Support for multi-table tournaments
\item \textbf{Reputation system}: Tracking player behavior to identify cheaters\cite{bfttheory}
\end{itemize}

\subsection{Personal Consideration}

This project provided valuable experience in cryptography\cite{ed25519}\cite{discrete-log-problem}, distributed systems\cite{spmd}\cite{mpi41}, blockchain technology\cite{eventtourcing}, and software engineering practices. The successful implementation demonstrates that complex theoretical concepts can be realized as practical, working systems\cite{Wei-Wang-Protocol}\cite{bfttheory}.
