...
\item \textbf{Game rule validation}: The proposal is passed to the poker manager, which validates turn order, action legality (e.g., sufficient chips for a bet) and compliance with Texas Hold'em rules.
\end{enumerate}
...
Even if some nodes are Byzantine (sending different votes or proposals), the quorum mechanism ensures that the supermajority of honest nodes drives the consensus outcome. The blockchain records the complete set of votes, enabling post-game auditing to identify Byzantine actors.
...
Message serialization uses JSON encoding for human readability and debugging convenience and then in binary to reduce bandwidth.
